    ...
    public function executeIndex($request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                //koiLib::debug(sfConfig::get('app_sf_guard_plugin_default_from_email', 'from@noreply.com'), true);

                $message = Swift_Message::newInstance()
                                ->setFrom(sfConfig::get('app_sf_guard_plugin_default_from_email', 'from@noreply.com'))
                                ->setTo($this->form->user->email_address)
                                ->setSubject(sfConfig::get('app_sf_guard_plugin_default_subject', 'Forgot Password Request for') . " " . $this->form->user->username)
                ...
                //$sendResult = $this->getMailer()->sendNextImmediately()->send($mensaje);

                if ($sendResult) {
                    $this->getUser()->setFlash('notice', 'Revise su correo electronico! Usted recibira su clave temporal en poco tiempo!');
                } else {
                    $this->getUser()->setFlash('error', 'Su clave temporal no pudo ser enviada');
                }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                $message = Swift_Message::newInstance()
                                ...
                                ->setSubject(sfConfig::get('app_sf_guard_plugin_default_confirm_change', 'New Password for ') . " " . $this->user->username)
